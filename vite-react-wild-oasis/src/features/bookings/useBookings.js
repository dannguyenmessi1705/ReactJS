...
export default function useBookings() {
  ...
  const { data: bookings, isLoading } = useQuery({
    queryKey: ["bookings", filter, sortBy], // Key để lưu cache, nếu key thay đổi thì dữ liệu sẽ được fetch lại, nên phải truyền filter vào key để khi filter thay đổi thì dữ liệu sẽ được fetch lại
    queryFn: () => getBookings(filter, sortBy), // Lấy ra danh sách booking từ API với điều kiện filter
  });
